...
class AnnotationSerializerForPaperInfo(serializers.ModelSerializer):
    ...
    class Meta:
        model = Annotation
        fields = ('pk', 'memo', 'comment_count', 'x0', 'y0', 'x1', 'y1', 'user', 'page', 'image_url', 'created_at')
...
class PaperInfoSerializer(serializers.ModelSerializer):
    ...
    def get_annotationed_users(self, obj):
        user_id_list = Annotation.objects.filter(paper=obj.pk, is_open=True).values_list('user', flat=True)
